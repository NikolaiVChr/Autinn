...
#define THRESHOLD_LIMIT_LOW_DB          -25.0//
...
struct Non : Module {
	...
	Non() {
		...
		configParam(Non::AVERAGE_TIME_PARAM, RMS_TIME_LOW_MS, RMS_TIME_HIGH_MS, RMS_TIME_DEFAULT_MS, "Average", " ms", 0.0f, 1.0f);
...
void Non::process(const ProcessArgs &args) {
	...
	if (tapKnob != params[ATTACK_PEAK_PARAM].getValue() || trKnob != params[RELEASE_PARAM].getValue() || rate != args.sampleRate || tavKnob != params[AVERAGE_TIME_PARAM].getValue() || gainKnob != params[OUT_GAIN_PARAM].getValue()) {
...
/*double Non::rms(double x, double TS) {
	double rms2 = (1.0 - TAV) * rms2_prev + TAV * x * x;
	rms2_prev = rms2;
	return rms2;
}*/

double Non::peak(double x, double TS, double ATp, double RT) {
	double xAbs = abs(x);
	if (xAbs > peak_prev) {
