...
struct Mixer6 : Module {
	...
	Mixer6() {
		...
		for (int ch = 0; ch < num_mono_channels; ch++) {
			...
			configParam(HIGH_PARAM+ch, 1/Pm, Pm, 1, "Channel "+std::to_string(ch+1)+" EQ High", " dB", -10.0f, 20.0f, 0);
			configParam(MID_PARAM+ch, 1/Pm, Pm, 1, "Channel "+std::to_string(ch+1)+" EQ Mid", " dB", -10.0f, 20.0f, 0);
			configParam(LOW_PARAM+ch, 1/Pm, Pm, 1, "Channel "+std::to_string(ch+1)+" EQ Low", " dB", -10.0f, 20.0f, 0);
