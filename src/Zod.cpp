...
struct Zod : Module {
	...
	double staticCurve(double rms, double peak, double LT, double LS, double CS, double CT, double CR, double NT, double ET, double ES, double ER, double knee);
...
double Zod::staticCurve(double rms, double peak, double LT, double LS, double CS, double CT, double CR, double NT, double ET, double ES, double ER, double knee) {
	limiter = false;
...
struct ZodWidget : ModuleWidget {
	ZodWidget(Zod *module) {
		...
		addParam(createParam<RoundMediumAutinnKnob>(Vec(16 * RACK_GRID_WIDTH*0.75-HALF_KNOB_MED, 250), module, Zod::OUT_GAIN_PARAM));
